...
class FlowVisualizerEvents(BaseVisualizer):
    def __init__(self, dataloader, save_path, clamp_flow=False, additional_args=None):
...
def visualize_optical_flow(flow, savepath=None, return_image=False, text=None, scaling=None):
    ...
    flow[numpy.isnan(flow)]=0
    # Use Hue, Saturation, Value colour model
    hsv = numpy.zeros((flow.shape[0], flow.shape[1], 3), dtype=float)

    # Calculate magnitude (remove the extra **0.5 which was causing issues)
    # Ensure flow values are finite before calculation
    flow_finite = numpy.isfinite(flow)
    flow_clean = numpy.where(flow_finite, flow, 0.0)
    mag = numpy.sqrt(flow_clean[...,0]**2+flow_clean[...,1]**2)
    # Ensure magnitude is finite
    mag = numpy.where(numpy.isfinite(mag), mag, 0.0)

    ang = numpy.arctan2(flow_clean[...,1], flow_clean[...,0])
    ang[ang<0]+=numpy.pi*2
    hsv[..., 0] = ang/numpy.pi/2.0 # Scale from 0..1
    # White background style: S = magnitude (0=white, 1=colorful), V = 1.0 (max brightness)
    if scaling is None:
        # Normalize magnitude to [0, 1] for saturation
        mag_max = numpy.nanmax(mag) if mag.size > 0 else 1.0
        if mag_max > 1e-6:  # Avoid division by zero
            mag_normalized = mag / mag_max
        else:
            mag_normalized = numpy.zeros_like(mag)
    else:
        # Clip and normalize using provided scaling
        mag_clipped = numpy.clip(mag, 0, scaling)
        mag_normalized = mag_clipped / scaling if scaling > 0 else numpy.zeros_like(mag)
    hsv[..., 1] = numpy.clip(mag_normalized, 0.0, 1.0)  # Saturation = magnitude (0=white, 1=colorful)
    hsv[..., 2] = 1.0  # Value = max brightness (white background style)
    
    # Ensure very small magnitudes are explicitly white (S=0, V=1)
    # Also handle any remaining invalid values
    very_small_mask = (mag < 1e-6) | (~numpy.isfinite(mag))
    hsv[very_small_mask, 1] = 0.0  # Force saturation to 0 for white background
    ...
    if savepath is not None:
        if plot_with_pyplot:
            ...
            out = bgr*255
            # Ensure background (very small or zero magnitude, or invalid) is white
            background_mask = (mag < 1e-6) | (~numpy.isfinite(mag))
            out[background_mask] = 255  # Force white background (RGB all 255)
            # Also ensure any NaN/Inf in output are white
            out = numpy.where(numpy.isfinite(out), out, 255)
            io.imsave(savepath, numpy.clip(out, 0, 255).astype('uint8'))
