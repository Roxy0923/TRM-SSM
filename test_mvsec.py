...
def warp_cv2(img, flow):
    return tools.warp_cv2(img, flow)

def mix_warp_image(img1, img2, alpha=0.5):
    return (img1 * alpha + img2 * (1 - alpha)).astype(img1.dtype)
...
class Test(object):
    ...
    def vis_event_image(self, event_voxel, name, mode='gray'):
        """
        将事件体素转换成清晰的累积图像（类似论文中的Reference）
        Args:
            event_voxel: [B, C, H, W] 事件体素
            name: 保存的文件名
            mode: 可视化模式 - 'gray'(灰度), 'hot'(热力图), 'jet'(彩虹), 'viridis'(紫蓝黄)
        """
        event_voxel = event_voxel[0].numpy()  # [C, H, W]
        C, H, W = event_voxel.shape
        
        accumulated = np.sum(np.abs(event_voxel), axis=0)  # [H, W]
        
        if accumulated.max() > 0:
            accumulated = accumulated / accumulated.max()
        
        accumulated = np.power(accumulated, 0.4)  # gamma=0.4 更强的对比度
        
        img_gray = (accumulated * 255).astype(np.uint8)
        ...
        if not os.path.exists(save_map_path):
            os.makedirs(save_map_path)
        cv2.imwrite(os.path.join(save_map_path, name), img_color)
        return
    ...
    def visualize_optical_flow_with_scale(self, flow, max_flow, name=None):
        """
        使用统一的最大光流尺度可视化，确保预测和GT的颜色映射一致
        
        Args:
            flow: 光流数组 [H, W, 2]
            max_flow: 统一的最大光流幅度（用于归一化）
            name: 保存的文件名
        """
        ...
        invalid_mask = nanIdx | idxUnknow
        u[invalid_mask] = 0
        v[invalid_mask] = 0

        zero_mask = np.sqrt(u ** 2 + v ** 2) < 1e-6

        u = u / (max_flow + np.finfo(float).eps)
        v = v / (max_flow + np.finfo(float).eps)

        img = compute_color(u, v)

        idx = np.repeat(invalid_mask[:, :, np.newaxis], 3, axis=2)
        img[idx] = 128  # 灰色，而不是0（黑色）

        img[np.repeat(zero_mask[:, :, np.newaxis], 3, axis=2)] = 255
    ...
    def visualize_optical_flow_reference(self, flow, name=None):
        """
        Visualize optical flow with white background (exactly same as training visualization).
        This generates reference images like those in the training folder.
        Uses the exact same logic as train_mvsec.py visualize_optical_flow method.
        """
        flow = flow.copy()
        ...
        background_mask = mag < 1e-6  # Very small magnitude threshold
                ...
                for batch_idx, batch in enumerate(self.data_loader):
                    ...
                    if visualize_map:
                        ...
                        if(batch_idx % 1 == 0):
                            ...
                            gt_valid = np.logical_and(
                                np.logical_and(~np.isinf(flow_gt[:, :, 0]), ~np.isinf(flow_gt[:, :, 1])),
                                np.linalg.norm(flow_gt, axis=2) > 0
                            )
                            est_valid = np.logical_and(
                                np.logical_and(~np.isinf(flow_est[:, :, 0]), ~np.isinf(flow_est[:, :, 1])),
                                np.abs(flow_est[:, :, 0]) < 1e7,
                                np.abs(flow_est[:, :, 1]) < 1e7
                            )
                            
                            combined_valid = gt_valid | est_valid
                            est_mag = np.sqrt(flow_est[..., 0]**2 + flow_est[..., 1]**2)
                            gt_mag = np.sqrt(flow_gt[..., 0]**2 + flow_gt[..., 1]**2)
                            if np.any(combined_valid):
                                est_mag_valid = est_mag[combined_valid]
                                gt_mag_valid = gt_mag[combined_valid]
                                unified_max_flow = max(
                                    np.percentile(est_mag_valid, 99) if len(est_mag_valid) > 0 else 1.0,
                                    np.percentile(gt_mag_valid, 99) if len(gt_mag_valid) > 0 else 1.0,
                                    1.0
                                )
                            else:
                                unified_max_flow = 1.0
                            
                            flow_est_viz = flow_est.copy()
                            flow_gt_viz = flow_gt.copy()
                            flow_est_viz[~est_valid] = np.nan
                            flow_gt_viz[~gt_valid] = np.nan
                            
                            if(print_epe):
                                self.visualize_optical_flow_with_scale(flow_est_viz, unified_max_flow, str(idx)+'_flow_est_{:.3f}.jpg'.format(AEE))
                            else:
                                self.visualize_optical_flow_with_scale(flow_est_viz, unified_max_flow, str(idx)+'_flow_est.jpg')

                            self.visualize_optical_flow_with_scale(flow_gt_viz, unified_max_flow, str(idx)+'_flow_gt.jpg')
                            self.visualize_optical_flow_with_scale(flow_gt_viz, unified_max_flow, str(idx)+'_reference.jpg')
...
class TestRaftEvents(Test):
    ...
    def get_estimation_and_target(self, batch):
        gt_key = 'fflow' if 'fflow' in batch.keys() else 'flow'
        gt_tensor = batch[gt_key].cpu().data

        if not self.downsample:
            if 'valid' in batch.keys():
                return batch['flow_est'].cpu().data, (gt_tensor, batch['valid'].cpu().data)
            return batch['flow_est'].cpu().data, gt_tensor
        else:
            f_est = batch['flow_est'].cpu().data
            f_gt = torch.nn.functional.interpolate(gt_tensor, scale_factor=0.5)
            if 'valid' in batch.keys():
                f_mask = torch.nn.functional.interpolate(batch['valid'].cpu().data, scale_factor=0.5)
                return f_est, (f_gt, f_mask)
            return f_est, f_gt
