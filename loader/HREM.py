...
class HREMEventFlow(Dataset):
    ...
    def get_data_ls(self):
        if(self.type == 'train'):
            ...
            # 添加调试信息
            print(f"[HREM Loader] Loading training data from: {self.dataset_dir}")
            print(f"[HREM Loader] proc_path = {proc_path}")
            print(f"[HREM Loader] event_interval = {self.dt}")

            self.data_ls = []
            # Iterate through scene directories (indoor_fast, indoor_slow, etc.)
            for scene in sorted(os.listdir(self.dataset_dir)):
                scene_dir = os.path.join(self.dataset_dir, scene)
                # 修复：支持符号链接，检查实际路径是否为目录
                if not (os.path.isdir(scene_dir) or (os.path.islink(scene_dir) and os.path.isdir(os.path.realpath(scene_dir)))):
                    continue

                scene_samples = []
                # Iterate through sample directories within each scene
                for names in sorted(os.listdir(scene_dir)):
                    sample_dir = os.path.join(scene_dir, names)
                    # 修复：支持符号链接，检查实际路径是否为目录
                    if not (os.path.isdir(sample_dir) or (os.path.islink(sample_dir) and os.path.isdir(os.path.realpath(sample_dir)))):
                        continue

                    event1_file_path = os.path.join(sample_dir, "events1.npz")
                    event2_file_path = os.path.join(sample_dir, "events2.npz")
                    flow_file_path = os.path.join(sample_dir, "flow.flo")

                    if(os.path.exists(event1_file_path) and os.path.exists(event2_file_path)):
                        self.data_ls.append({"names":names, "event0":event1_file_path, "event1":event2_file_path, "fflow":flow_file_path, "scene":scene})
                        scene_samples.append(names)

                print(f"[HREM Loader] Scene '{scene}': {len(scene_samples)} samples")

            print(f"[HREM Loader] Total training samples loaded: {len(self.data_ls)}")
            ...
            # 添加调试信息
            print(f"[HREM Loader] Loading test data from: {self.dataset_dir}")

            self.nori_list = {}
            self.data_ls = []  # flatten all test samples by default
            for sequence in sequence_list:
                self.dataset_sequence_dir = os.path.join(self.dataset_dir, sequence)
                # 修复：支持符号链接
                if not (os.path.isdir(self.dataset_sequence_dir) or (os.path.islink(self.dataset_sequence_dir) and os.path.isdir(os.path.realpath(self.dataset_sequence_dir)))):
                    continue

                data_ls = []
                for names in sorted(os.listdir(self.dataset_sequence_dir)):
                    sample_dir = os.path.join(self.dataset_sequence_dir, names)
                    # 修复：支持符号链接
                    if not (os.path.isdir(sample_dir) or (os.path.islink(sample_dir) and os.path.isdir(os.path.realpath(sample_dir)))):
                        continue
                    ...
                    if(os.path.exists(event1_file_path) and os.path.exists(event2_file_path)):
                        data_ls.append({"names":names, "event0":event1_file_path, "event1":event2_file_path, "fflow":flow_file_path, "scene":sequence})

                print(f"[HREM Loader] Test sequence '{sequence}': {len(data_ls)} samples")
                self.nori_list.update({sequence:data_ls})
                self.data_ls.extend(data_ls)

            print(f"[HREM Loader] Total test samples loaded: {len(self.data_ls)}")
    ...
    def __len__(self):
        ...
        # Ensure data_ls exists; for test mode, it is flattened by default
        if hasattr(self, 'data_ls'):
            return len(self.data_ls)
        # Fallback: count from nori_list if present
        if hasattr(self, 'nori_list'):
            return sum(len(v) for v in self.nori_list.values())
        return 0
    ...
    def get_sample(self, idx):
        ...
        names = sample["names"]
        scene = sample.get("scene", "unknown")  # 获取序列类型（indoor_fast, outdoor_fast等）
