...
class FlowFormer(nn.Module):
    def __init__(self, cfg, input_dim=3):
        ...
        self.memory_encoder = MemoryEncoder(cfg, input_dim=input_dim)
        self.memory_decoder = MemoryDecoder(cfg)
        # if cfg.cnet == 'twins':
        #     self.context_encoder = twins_svt_large(pretrained=self.cfg.pretrain)
        # elif cfg.cnet == 'basicencoder':
        #     self.context_encoder = BasicEncoder(output_dim=256, norm_fn='instance', input_dim=input_dim)
    ...
    # 兼容外部调用传入的 step_scale（训练框架会为 SSM 模型传该参数）
    # FlowFormer 未使用 step_scale，因此仅接受并忽略，避免报错
    def forward(self, events1, events2, output=None, flow_init=None, step_scale=None, **kwargs):
