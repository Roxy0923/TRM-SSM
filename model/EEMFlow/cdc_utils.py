...
# Add RVT models to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../..'))
from RVT.models.adaptive_ssm import AdaptiveSSM2DRefiner
...
class cdc_ssm_model(nn.Module):
    """
    CDC model with SSM-based refinement for frequency-adaptive optical flow.

    This variant adds AdaptiveSSM2DRefiner to refine the inter_flow predictions
    from the dense estimator, enabling frequency-adaptive processing based on
    dataset temporal resolution (dt1/dt4 settings).

    Args:
        step_scale (float): Fixed step_scale value from dataset config.
        ...
        ssm_state_dim (int): SSM state dimension. Default: 64 (论文推荐：60Hz输入使用64-128)
        blend_weight (float): Blending weight for SSM refinement.
            refined = blend_weight * ssm_refined + (1 - blend_weight) * cnn_flow
            Start conservatively (0.3) and increase based on validation.
            Default: 0.3
        dropout (float): Dropout probability for SSM. Default: 0.1
    """

    def __init__(self, step_scale=1.0, ssm_state_dim=64, blend_weight=0.3, dropout=0.1, anti_alias=True, h2_reg_weight=0.0):
        super(cdc_ssm_model, self).__init__()
        # Cache for inference-time step scaling (dt_test / dt_train)
        self.step_scale = step_scale
        self.h2_reg_weight = h2_reg_weight
        self.last_h2_reg = torch.tensor(0.0)
        # Debug: Print h2_reg_weight to verify it's set correctly
        import sys
        if h2_reg_weight > 0:
            print(f"[cdc_ssm_model] H2 regularization enabled with weight={h2_reg_weight}", flush=True)
        else:
            print(f"[cdc_ssm_model] H2 regularization disabled (h2_reg_weight={h2_reg_weight})", flush=True)
        ...
        # **NEW: Add SSM refiner for frequency-adaptive processing**
        # The feature tensor has 184 channels (accumulated from all conv layers: 32+32+32+16+8+64)
        num_feature_channels = sum(f_channels_es) + in_C  # 184
        self.ssm_refiner = AdaptiveSSM2DRefiner(
            channels=num_feature_channels,  # Match actual feature channels
            state_channels=ssm_state_dim,
            dropout=dropout,
    ...
    def set_step_scale(self, step_scale: float):
        """Dynamically update step_scale for frequency adaptation without reinstantiating the model."""
        self.step_scale = step_scale

    def forward(self, flow_init, feature_1, feature_2, output_level_flow=None, return_reg: bool = False, step_scale: float | None = None):
        """
        Forward pass with SSM-based flow refinement.

        Args:
            flow_init: [B, 2, H, W] - Initial flow estimate
            feature_1: [B, C, H, W] - Current frame feature
            feature_2: [B, C, H, W] - Previous frame feature
            output_level_flow: Optional target resolution flow

        Returns:
            flow_init: [B, 2, H, W] - Upsampled initial flow
            flow_up: [B, 2, H, W] - Refined flow
            inter_flow: [B, 2, H, W] - Intermediate flow
            inter_mask: [B, 1, H, W] - Confidence mask
        """
        n, c, h, w = flow_init.shape
        n_f, c_f, h_f, w_f = feature_1.shape

        # Upsample flow_init to match feature resolution
        if h != h_f or w != w_f:
            flow_init = upsample2d_flow_as(flow_init, feature_1, mode="bilinear", if_rate=True)

        # Warp feature_2 with current flow estimate
        feature_2_warp = self.warping_layer(feature_2, flow_init)

        # Dense estimator: Predict inter_flow and confidence mask
        input_feature = torch.cat((feature_1, feature_2_warp), dim=1)
        feature, x_out = self.dense_estimator_mask(input_feature)
        
        # **NEW: Apply SSM refinement to intermediate features (before flow extraction)**
        # SSM refines the feature representation, not the flow directly
        # Apply SSM refinement with correct step_scale to adapt frequency
        dynamic_step_scale = step_scale if step_scale is not None else self.step_scale
        try:
            # Always compute H2 reg if h2_reg_weight > 0, regardless of return_reg flag
            # This ensures last_h2_reg is always updated for training loop access
            if self.h2_reg_weight > 0:
                feature_refined, h2_reg = self.ssm_refiner(feature, step_scale=dynamic_step_scale, return_reg=True)
                # Ensure h2_reg is on the same device as feature and is a scalar tensor
                if torch.is_tensor(h2_reg):
                    self.last_h2_reg = h2_reg.to(feature.device)
                else:
                    self.last_h2_reg = torch.tensor(h2_reg, device=feature.device, dtype=feature.dtype)
            else:
                feature_refined = self.ssm_refiner(feature, step_scale=dynamic_step_scale)
                self.last_h2_reg = torch.tensor(0.0, device=feature.device, dtype=feature.dtype)
            # Blend SSM-refined features with CNN features
            feature = self.blend_weight * feature_refined + (1 - self.blend_weight) * feature
            
            # Re-extract flow from refined features using the last conv layer
            x_out = self.dense_estimator_mask.conv_last(feature)
        except Exception as e:
            # Log error with full details
            error_msg = f"ERROR: SSM feature refinement failed ({type(e).__name__}: {e}), using CNN features only"
            print(error_msg, flush=True)
            import traceback
            traceback.print_exc()
            # On exception, set last_h2_reg to 0
            self.last_h2_reg = torch.tensor(0.0, device=feature.device, dtype=feature.dtype)
            # Note: We continue training with CNN features only, but this should be fixed
            # The error has been fixed by adding .contiguous() calls, so this should not occur anymore
        ...
        # Final fusion with confidence masking
        flow_up = tensor_tools.torch_warp(flow_init, inter_flow) * (1 - inter_mask) + flow_init * inter_mask

        if return_reg:
            return flow_init, flow_up, inter_flow, inter_mask, self.last_h2_reg
        return flow_init, flow_up, inter_flow, inter_mask
