...
def discretize_bilinear(Lambda, B_tilde, Delta):
    ...
    """
    # Handle vmap compatibility: ensure contiguous for proper storage_offset
    Lambda = torch.view_as_complex(Lambda.clone().contiguous())
...
class S5SSM(torch.nn.Module):
    def __init__(
        ...
        # Debug counters for early-step frequency/mask sanity check
        self._debug_counter = 0
        self._debug_limit = 100
    ...
    def forward_rnn(self, signal, prev_state, step_scale: float | torch.Tensor = 1.0):
        ...
        C_tilde = self._apply_bandlimit(C_tilde, step_scale)
    ...
    def _apply_bandlimit(self, C_tilde: torch.Tensor, step_scale: float | torch.Tensor):
        """Apply bandlimit mask on C using current step_scale (dynamic, no parameter overwrite)."""
        if self.bandlimit is None:
            return C_tilde

        # CRITICAL FIX: Clone tensor before any operations to avoid vmap storage_offset issues
        # When called inside torch.vmap, parameter references may have incompatible storage layout
        C_tilde = C_tilde.clone()

        # Resolve step_scale to a scalar for frequency computation
        if torch.is_tensor(step_scale):
            if step_scale.ndim == 0:
                scale = step_scale
            else:
                scale = step_scale.reshape(-1)[0]
        else:
            scale = step_scale
        step = scale * torch.exp(self.log_step)
        freqs = step * self.Lambda[:, 1].abs() / (2 * math.pi)
        mask = (freqs < self.bandlimit * 0.5).float()  # Nyquist频率阈值

        # Now safe to call view_as_complex on cloned tensor
        C_complex = torch.view_as_complex(C_tilde)
        C_complex = C_complex * mask

        # Debug: print freqs/mask stats for early steps to detect over-masking
        # 仅在非 vmap 场景打印（step_scale 不是 Tensor），避免 .item() 触发 vmap 限制
        if self._debug_counter < self._debug_limit and (not torch.is_tensor(step_scale)):
            with torch.no_grad():
                freqs_det = freqs.detach().contiguous()
                mask_det = mask.detach().contiguous()
                f_max = freqs_det.reshape(-1).max().float().cpu().item()
                f_mean = freqs_det.reshape(-1).mean().float().cpu().item()
                m_mean = mask_det.reshape(-1).mean().float().cpu().item()
                scale_val = float(scale)
                print(f"[S5SSM][debug {self._debug_counter}] "
                      f"step_scale={scale_val:.4f} "
                      f"freqs_max={f_max:.4f} freqs_mean={f_mean:.4f} "
                      f"mask_mean={m_mean:.4f}", flush=True)
            self._debug_counter += 1

        return torch.view_as_real(C_complex)
...
class S5Block(torch.nn.Module):
    def __init__(
        ...
        # Scalar-bandlimit debug (outside vmap) - first few calls only
        self._bl_debug_cnt = 0
        self._bl_debug_max = 5

    def forward(self, x, states, step_scale: float | torch.Tensor = 1.0):
        # Standard transfomer-style block with GEGLU/Pre-LayerNorm
        fx = self.attn_norm(x)
        res = fx.clone()
        # === Bandlimit scalar debug (only when step_scale is scalar, before vmap) ===
        if (not torch.is_tensor(step_scale)) and self._bl_debug_cnt < self._bl_debug_max:
            seq = self.s5.seq  # S5SSM
            if getattr(seq, "bandlimit", None) is not None:
                with torch.no_grad():
                    ...
                    print(f"[S5Block][debug {self._bl_debug_cnt}] "
                          f"step_scale={float(step_scale):.4f} "
                          f"freqs_max={fmax:.4f} freqs_mean={fmean:.4f} "
                          f"mask_mean={mmean:.4f}", flush=True)
                self._bl_debug_cnt += 1

        # Pass step_scale so SSM discretization matches inference frequency
        x, new_state = self.s5(fx, states, step_scale=step_scale)
        x = F.gelu(x) + res
