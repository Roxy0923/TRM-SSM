...
# 自定义 ZipperLongest 类，用于替代 torchdata.datapipes.iter.ZipperLongest
# 原因：PyTorch 2.2.1 中没有 ZipperLongest，需要自己实现
class ZipperLongest(IterDataPipe):
    """使用 itertools.zip_longest 实现的 ZipperLongest DataPipe"""
    def __init__(self, *datapipes, fill_value=None):
        super().__init__()
        self.datapipes = datapipes
        self.fill_value = fill_value
    
    def __iter__(self):
        return zip_longest(*self.datapipes, fillvalue=self.fill_value)
...
class ShardedStreamingDataPipe(IterDataPipe):
    ...
    def __iter__(self):
        ...
        # 使用 itertools.cycle 和 zip 替代旧版 torchdata 的方法
        # 原因：新版 PyTorch 的 IterableWrapper 没有 cycle 和 zip 方法
        worker_id_stream = cycle([local_worker_id])
        return iter(zip(zipped_stream, worker_id_stream))
